...
adds a small source-aware guard to the recipe's post-source hook, creating
that hook for older recipes that do not define one.
"""
...
def prepare(path: pathlib.Path) -> bool:
    ...
    if POST_SOURCE_OPEN_RE.search(text):
        # Keep the guard inside the existing termux_step_post_get_source.  It
        # runs after CPython has been extracted, but before
        # termux_step_patch_package applies the recipe's patches.
        ...
        # Older Termux Python recipes have no post-source hook.  A new hook at
        # this location has the same ordering and preserves their configure
        # hook unchanged.
